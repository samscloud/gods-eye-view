...
test('expanded left panels integrate their headers with the container shell', () => {
  ...
  assert.match(
    css,
    /#left-panel-stack > \[data-panel-id\]:not\(\.collapsed\) \.panel-divider\s*\{[\s\S]*?linear-gradient\(\s*90deg,\s*rgba\(var\(--accent-rgb\), 0\.28\),\s*rgba\(var\(--accent-rgb\), 0\.18\) 58%,\s*transparent\s*\)[\s\S]*?box-shadow:\s*0 0 7px rgba\(var\(--accent-rgb\), 0\.22\);/,
  );
});
...
test('expanded right panels highlight the title divider without changing collapsed launchers', () => {
  ...
  assert.match(
    css,
    /#right-context-rail \[data-panel-id\]:not\(\.collapsed\) \.panel-divider\s*\{[\s\S]*?linear-gradient\(\s*90deg,\s*rgba\(var\(--accent-rgb\), 0\.28\),\s*rgba\(var\(--accent-rgb\), 0\.18\) 58%,\s*transparent\s*\)[\s\S]*?box-shadow:\s*0 0 7px rgba\(var\(--accent-rgb\), 0\.22\);/,
  );
  assert.match(
    css,
    /#param-slider-panel:not\(\.collapsed\) \.param-panel-divider\s*\{[\s\S]*?linear-gradient\(\s*90deg,\s*rgba\(var\(--accent-rgb\), 0\.28\),\s*rgba\(var\(--accent-rgb\), 0\.18\) 58%,\s*transparent\s*\);/,
  );
});
